...
const EditCommunityAdmin = () => {
    ...
    useEffect(() => {
      async function fetchProducts() {
        const { data } = await axios.get(
          `http://127.0.0.1:8000/admin-statistic/all-community-engagment-activity/${id}`,
          {
            headers: {
    ...
    const submitHandler = (e) => {
      ...
      axios
        .put(`http://127.0.0.1:8000/admin-statistic/all-community-engagment-activity/${id}/`, payload, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
